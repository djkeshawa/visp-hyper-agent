...
description: >-
  ...
  running tests/builds/typecheck and triaging their output, and simple
  deterministic edits (renames, import fixes, formatting, boilerplate). Do NOT
  use for tasks that require design decisions or non-trivial logic.
...
- **Run & report**: execute `pnpm test`, `pnpm typecheck`, `pnpm build`, or
  `visp` commands; report pass/fail with the exact failing output, trimmed.
