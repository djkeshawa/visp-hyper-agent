...
- **MCP surface manifest**: implemented as `visp-hyper://meta/surface-manifest`; it exposes a stable SHA-256 over the fixed tool/resource/prompt surface, per-tool input/output-schema hashes, structured tool result guarantees, and safety posture so MCP clients and enterprise reviewers can detect unexpected orchestration drift.
