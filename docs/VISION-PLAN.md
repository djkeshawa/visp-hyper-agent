...
- **Context manifest**: implemented as `.visp/hyper/current/context-manifest.json`; it gives weaker agents and MCP clients one machine-readable contract for required reads, resource URIs, selected files, Kit artifact provenance hashes, validation commands, blocked paths, relevant failure patterns, and the pinned Kit context artifact hash. It now copies Visp Kit contract `1.3` typed orchestrator read artifacts and freshness policy into active handoffs when available. `checkpoint --task` fails closed when the adopted context pack or any grounded Kit provenance artifact changes before validation.
...
- **MCP surface manifest**: implemented as `visp-hyper://meta/surface-manifest`; it exposes a stable SHA-256 over the fixed tool/resource/prompt surface, per-tool input/output-schema hashes, structured tool result guarantees, and safety posture so MCP clients and enterprise reviewers can detect unexpected orchestration drift. Hyper also exposes `visp-hyper://current/context-freshness` and `visp-hyper://current/kit-read-contract` as always-available computed JSON resources so hosts can block stale context, inspect Kit artifact roles, and follow freshness policy without scraping CLI prose.
