...
| `visp-hyper doctor [--json]` | Read-only compatibility check for the Hyper + Visp Kit chain: Hyper state, Kit artifacts, `visp --json` parsing, Kit contract capabilities, policy validation, next gate, active task context pack, and the git scope hook. |
