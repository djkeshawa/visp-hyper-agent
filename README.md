...
| `visp-hyper doctor [--json]` | Read-only compatibility check for the Hyper + Visp Kit chain: Hyper state, active context freshness, Kit artifacts, `visp --json` parsing, Kit contract capabilities including provenance freshness, policy validation, next gate, active task context pack, git scope hook, and MCP surface manifest hash. |
