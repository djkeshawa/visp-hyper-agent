...
`visp-hyper://current/canonical-action` computes the current validated Kit action on every read. Healthy Kit-backed projects return the negotiated canonical action; genuine Kit absence and configured failures return distinct unavailable or inconclusive states without a local authority fallback.
